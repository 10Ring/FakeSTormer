...
def facecrop(model, org_path, save_path, period=1, num_frames=10, dataset='original', label=None, mask_path=None, padding=PADDING):
    ...
    if label is not None:
        ...
        save_path_ = save_path + f'/{DATA_TYPE}/{str(label)}/' + os.path.basename(org_path).replace('.mp4','/')
    else:
        save_path_ = save_path + f'/{DATA_TYPE}/{dataset}/' + os.path.basename(org_path).replace('.mp4','/')
...
if __name__=='__main__':
    ...
    parser.add_argument('-d', dest='dataset', 
                              choices=['FaceShifter','Face2Face','Deepfakes','FaceSwap','NeuralTextures',\
                                       'Original','Celeb-real','Celeb-synthesis','YouTube-real','DFDC', \
                                       'DFDCP','method_A','method_B','original_videos', \
                                       'DFo_source_videos', 'DFo_manipulated_videos', 'ForgeryNet'])
    ...
    if args.dataset in ['Original','DeepFakeDetection','FaceShifter','Face2Face','Deepfakes','FaceSwap','NeuralTextures']:
        ...
        with open(f'{dataset_path}lists/splits/{args.task}.txt') as f:
            ...
            if args.dataset == 'DFo_source_videos':
                vid_ids = list(set([vid_id.split('_')[1].split('.')[0] for vid_id in vid_ids]))
    elif args.dataset in ['ForgeryNet']:
        movies_path = os.path.join(dataset_path, args.task, 'video')
        
        # Annotation file for FGN
        vid_ids = []
        with open(os.path.join(dataset_path, args.task, 'video_list.txt'), encoding='utf-8') as f:
            ...
            for idx, row in enumerate(df):
                ...
                if int(row_data[-1]) in [0, 7] :
                    ...
                    label = f'fake_{args.task}' if int(row_data[-1]) == 7 else f'real_{args.task}'
                    labels.append(label)
    else:
        movies_path = dataset_path
    ...
    for i in range(len(vid_ids)):    
        if args.dataset == 'Original':   
            ...
            for item in full_file_list:
                if (vid_ids[i] in vid_id_count.keys() and vid_id_count[vid_ids[i]] > 10): break
