...
def randaffine(img, mask, index=0, data_type='image', **kwargs):
    ...
    if data_type == 'image':
        ...
        f = alb.ReplayCompose([
            alb.Affine(translate_percent={'x':(-0.03,0.03),'y':(-0.015,0.015)}, 
                       scale=[0.95,1/0.95], 
                       fit_output=False, p=1)
        ], p=1)

        g = alb.ReplayCompose([
            alb.ElasticTransform(alpha=50, 
                                 sigma=7, 
                                 alpha_affine=0, 
                                 p=1)
        ], p=1)
    ...
    if index == 0 or data_type=='image':
        ...
        if data_type == 'image':
            ...
            return img, mask, {'f_replay_params': f_replay_params, 'g_replay_params': g_replay_params}
    else:
