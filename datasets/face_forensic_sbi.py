...
class SBIFaceForensic(MasterDataset):
        ...
        while flag:
            try:
                ...
                if self.debug:
                    ...
                    Image.fromarray(mvp_f_drawed).save(f'samples/debugs/mvp_f_{idx}.jpg')
    ...
    def train_collate_fn(self, batch):
        ...
        # label = torch.tensor([0] * len(img_r) + [1]*len(img_f))
        cst = torch.cat([torch.tensor(np.array(cst_r)).float(), torch.tensor(np.array(cst_f)).float()], 0)
...
if __name__=="__main__":
    ...
    for b, batch_data in enumerate(hm_ff_loader):
        inputs, labels, heatmaps, consistencies = batch_data["img"], batch_data["label"], batch_data["heatmap"], batch_data["cstency"]
